...
在 `config.yaml` 同目录创建固定文件 `config.strategy.yaml`（从公开模板 `config.strategy.example.yaml` 复制改名，模板全是虚构示例值）。该文件没有命令行替代参数，并由 `.gitignore` 排除；真实策略开关、阈值和部署参数只保留在交易机本地，不得提交。缺失、关闭、字段拼错或与主配置冲突都会直接阻止启动。本机 `deploy/<机器名>/` 目录（不入库）还留有各交易机的部署配置对：可以把整个 `deploy/` 目录一次复制到交易机，启动时直接 `python main.py <机器名>`（如 `python main.py elio_gj`）按机器名选择配置（`config.strategy.yaml` 自动读取同目录、必须成对），机器名拼错会列出所有可选机器；之后更新配置只需覆盖 `deploy/`、不用重拷代码目录。详见 `deploy/部署说明.md`。
...
| `09:30:00` 起 | **补仓窗口**：核对全部在途买单后，新查可用现金，保留底线以上的首波余款、退款和卖出回款均可分波补仓；继续受仓位、现金预留、最小手数和波间隔限制 | `opening_buy.admit_until` 收口（示例 `09:35:00`） |
...
2. **余款和回款补仓（第二波起）**：保留底线 = 首波前可用现金 − 首波实际接纳预算；补仓池 = 新查询的可用现金 − 保留底线，下限为 0。首波预算内没花完的钱、确认撤单后的退款和卖出回款均可再分配，原本未分配的现金继续保留。普通买单未收口，或事件已落库但 worker 尚未登记 signal 时，本轮不补；涨停排队单的委托账本和冻结现金核对通过后，只排除该代码，并把未成交本金计入总仓位占用。补仓继续受总仓位、单票上限、现金预留、最小手数、波间隔和 `admit_until` 限制。券商的新可用现金必须已扣除所有有效委托冻结，全账户冻结金额只能做一致性检查，不能证明逐单归属。
3. **波次编号与重启**：每波信号 id 带波次序号（`...-topup01-...`、`topup02`…），从当日已落库的补仓信号数重建。重启后不会重发旧波，接着从下一波继续。
4. **不足最小手数就留池**：某只候选分到的预算达不到 `topup_min_wave_lots`（默认 3 手，含报价余量），本轮不发，资金留池和后续回款一起评估。
5. **新旧资金状态**：新保留底线按交易日写入已有 `strategy_state` 的 `opening_buy_cash_floor_v2`。只写底线、尚未写批次事件就中断时，新查资金重算首波；当日已有首波事件却只有旧 `wave1_cash_baseline` 时，停止新增补仓并告警，已接纳委托恢复和卖出流程继续。建议新交易日启用，不换算旧值、不回退执行账本。

涨停排队买单部分成交时，现有慢轮询会把递增的成交数量写入委托账本，仍保持排队状态。补仓只计剩余挂单本金；重复或回退的数量不重复写库。更新受原排队轮询节奏限制，账本与冻结资金暂未对上时先等待下一轮。
...
| 配置 | 说明 |
| --- | --- |
| `execution.quote_band_pct` | 百分比包络：用于 slippage、盘前排队及原开盘回退路径（默认 1.5%）；开盘后有合格新鲜卖一的 book 买单优先使用卖一加 tick，不再被包络覆盖 |
...
| `execution.quote_max_age_sec` | 执行 Quote 使用本次 tick 的源时间；连续竞价超龄有界重取，耗尽仍沿原告警路径。book 买单不再享受竞价旧帧的开盘宽限；0=关闭时效门控且不启用新鲜盘口优先分支（建议 3） |
...
| `execution.opening_price_gap_wait_pct` / `opening_price_gap_wait_max_sec` / `opening_price_gap_wait_poll_sec` | 开盘首挂等待：book 比较本帧新鲜卖一，其他模式保留最新价判断；超过阈值则交撤单确认，未知盘口有界等待。默认最多 6s、每 0.2s 重判；阈值或等待上限为 0 关闭，poll 为 0 使用原 1s 间隔 |
...
- **开盘首挂有耐心**：开盘窗口内首笔买单超时未成交时，book 模式看本帧新鲜卖一；卖一甩开阈值就结束额外等待，交原撤单确认流程。价格贴近或没有可靠盘口时有界等待；竞价旧帧与可成交侧零成交仍保留一次 ghost 核验。
...
- **行情与快照防呆**：行情取数失败退避重试 3 次；执行 Quote 不再用另一帧推送到达时间覆盖本帧源时间。超龄重取有上限，不能保证一定取到新行情。策略 MarketSnapshot 暂保留原时效语义；源时间不推进的标的可能增加执行重取耗时，需要在交易机验证。
...
本地工具 `tools/analyze_buy_costs.py` 保留 `--db`、`--logs`、`--code` 参数。一次只传入一个账户、同一交易日的账本和日志；数据库以只读方式打开，路径不存在时不会新建空库。

新日志用完整 `signal_id`、委托号和 attempt 序号关联定价，用成交 ID 去重，允许成交回调早于定价日志。没有可靠身份的旧定价日志不猜卖一归属。报告分别列账本成交量、价格覆盖、卖一基准覆盖、每次委托成本和相对首笔卖一的整信号成本；缺失或冲突数据不进入完整成本样本。

旧成交日志没有成交 ID 时，只展示观察均价，并标记“覆盖未核实”；即使带价股数恰好等于账本成交量，也不能证明没有重复或漏记。冲突成交 ID 的价格和数量不择一采用，冲突耗时不进入分位数。

速度分为本地锁等待、SDK 调用、执行开始到取得委托号、提交日志到成交回调日志。SDK 调用在 `order_stock()` 前后计时；取得委托号是执行层可见的适配器返回点，包含适配器处理，均不代表交易所收到订单。回调早于提交日志时不计算该间隔。日志需要保留这些 DEBUG 行；没有足够覆盖率和样本数，不能用报告认定提速或滑点改善。
...
| 补仓池 | 新查可用现金减保留底线；首波预算余款、退款和卖款都可参与，仍受在途与仓位约束 |
| 保留底线 | 首波前可用现金减首波已接纳预算，按交易日保存为 `opening_buy_cash_floor_v2` |
